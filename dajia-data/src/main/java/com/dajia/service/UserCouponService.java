...
import static com.dajia.domain.CouponConstants.STATUS_EXPIRED;
import static com.dajia.domain.CouponConstants.STATUS_GIVEUP;
...
public class UserCouponService {
	...
	final static List<Integer> STATUS_SHOULD_NOT_REQUEST_AGAIN = Arrays.asList(STATUS_ACTIVE, STATUS_EXPIRED,
			STATUS_USED, STATUS_GIVEUP);
	...
	public DajiaResult publishCoupons(Long couponId, Integer amount, List<Long> users, String createdBy) {
		...
		String userMsg = totalSuccess == total ? "优惠券批量发放全部成功" : String.format("优惠券批量发放部分成功, 预计发放%d, 成功%d, 失败%d",
				total, totalSuccess, total - totalSuccess);
	 ...
	 * 用户是否可以领取某张优惠券
	 *
	 * 如果有状态为 1. 已发放 2. 已使用 3. 已过期 4. 自己主动放弃
	 *
	 * 则视为已经领取过优惠券
	 */
	public DajiaResult canRequest(Long userId, Long couponId) {
		...
		DajiaResult result;
		try {
			int userAlreadyHave = userCouponRepo.countByUserIdAndCouponIdAndStatusIn(userId, couponId,
...
//			}
	...
	public DajiaResult getUserCouponsByStatus(Long userId, List<Integer> statusList) {
		...
		try {
			return DajiaResult.successReturn(COMMON_MSG_QUERY_OK, null,
					userCouponRepo.findByUserIdAndStatusIn(userId, Arrays.asList(STATUS_ACTIVE)));
		} catch (Exception ex) {
	...
	public DajiaResult getAvailableConponsWhenBuy(Long userId) {
		...
		/**
		 * 规则: 1. UserId匹配 2. 订单ID为空 3. status为可使用 4. 限制使用开始时间 <= now 5.
		 * 限制使用过期时间 >= tomorrow morning
		 */
		try {
			List<UserCoupon> availableCoupons = userCouponRepo
					.findByUserIdAndStatusInAndOrderIdIsNullAndGmtExpiredGreaterThanEqualAndGmtStartLessThanEqual(
							userId, Arrays.asList(STATUS_ACTIVE), tomorrowMorning, now);
	...
	public DajiaResult getTotalCutOffWithCoupons(List<Long> couponPkList, Long userId) {
		 ...
		 * 三种异常
		 *
		 * 1. 伪造优惠券ID 2. 使用别人的优惠券ID 3. 优惠券状态不正常
		 */
		List<UserCoupon> userCoupons;
		try {
			userCoupons = userCouponRepo.findByUserIdAndIdInAndStatus(userId, couponPkList,
